...
function detectWeb(doc, url) {
    const a = doc.evaluate('//span[@id="education"]', doc).iterateNext().textContent;
    if (a !== null) {
        return "thesis";
    } else {
        return false;
...
function scrape(doc, url) {
    if (detectWeb(doc, url) === "thesis") {
        const newItem = new Zotero.Item('thesis');
        const title = doc.getElementById("title_cn").textContent;
        const teacher = doc.getElementById("teacher_name").childNodes[1].textContent;
        const author = doc.getElementById("author_name").childNodes[1].textContent;
        doc.getElementById("more_abstract_cn").click(); //模拟点击显示更多摘要
        const abstract = doc.getElementById('abstract_cn').textContent;
        var iso = doc.getElementById('education_grant_time').textContent; //毕业时间作为论文发表时间
        const sid = doc.getElementById('fullText').children[0].href.match("\'(.*?)\'")[0]; //获取学生学号
        let pdfurl = "http://dpaper.las.ac.cn/Dpaper/detail/getFile?student_no=" + sid + "&type=all";
